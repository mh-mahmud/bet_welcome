...
class Adminlogin extends CI_Controller {
	...
	public function authenticate() {
		...
		$result = $this->admin_login_model->authentication($data);
		...
		if(!empty($result) > 0) {
		     $this->session->set_userdata('admin_access_data', $result);
		     redirect('/admin');
		}
		else {
		     $this->session->set_flashdata('error', 'Incorrect Crediantials');
	...
	public function authenticate_otp()
	{
		...
		$data['email'] = $this->input->post('email');
		$data['password'] = $this->input->post('password');
		$result = $this->admin_login_model->authentication_otp($data);
